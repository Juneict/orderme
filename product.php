<?php 
    include "db.inc.php";
    $query ="SELECT * FROM products ORDER BY order_date DESC";
    ...
    if($_SERVER['REQUEST_METHOD'] =='POST'){
        ...
        //$order_date =now();
        // echo $order_date;
        $sql = "INSERT INTO `products`(`product`, `categories`, `brand`, `qty`, `price`) VALUES ('$product','$categories','$brand','$qty','$pricr')";
        $results = mysqli_query($conn,$sql);

        if(!$results){
            die("Failed");
       }else{
           header("location:productlist.php?product-added");
       }
    }
    if(isset($_GET['delete_todo'])){
        $delete =$_GET['delete_todo'];
        $sqli ="DELETE FROM products WHERE id=$delete";
        $res =mysqli_query($conn,$sqli);
        if(!$res){
            die("Failed");
       }else{
           header("location:productlist.php?product-deleted");
       }
    }
// ?>
<!DOCTYPE html>
                        ...
                        <div class="nav">
                            ...
                            <div class="collapse" id="collapsePages" aria-labelledby="headingTwo" data-parent="#sidenavAccordion">
                                <nav class="sb-sidenav-menu-nested nav accordion" id="sidenavAccordionPages">
                                    <a class="nav-link collapsed" href="order.php" data-toggle="collapse" data-target="#pagesCollapseAuth" aria-expanded="false" aria-controls="pagesCollapseAuth">
                                        အမှာစာအသစ်
                                    </a>
                    ...
                    <div class="container-fluid">
                        ...
                        <form action="<?php echo $_SERVER['PHP_SELF'];?>" method="POST">
                            <div class="form-group">
                                <input class="form-control" type="text" name="product" placeholder="ပစ္စည်း အမည်">                   
                            </div>
                            <div class="form-group">
                                <input class="form-control" type="text" name="categories" placeholder="ပစ္စည်း အမျိုးအစား"> 
                            </div>
                            <div class="form-group">
                                <input class="form-control" type="text" name="brand" placeholder="ပစ္စည်း အရွယ်အစား">
                            </div>
                            <div class="form-group">
                                <input class="form-control" type="text" name="qty" placeholder="အရေအတွက်">
                            </div>
                            <div class="form-group">
                                <input class="form-control" type="text" name="price" placeholder="ဈေးနှုန်း">
                            </div>
                            <div class="from-group">
                                <input class="btn btn-primary" type="submit" value="ထည့်ရန်">
                            </div>
                        </form>
                    </div>
                            </div>
